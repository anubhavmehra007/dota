...
def get_hero_list():
    ...
    try:
        ...
        if res.status_code == 200:
            ...
            hrefs = [link.get('href') for link in links if not (
                link.get('href') == '/Intelligence' or link.get('href') == '/Agility')]
...
def get_hero_response(hero):
    try:
        ...
        if res.status_code == 200:
            ...
            responses_link = [link for link in links if len(
                link.find_all('audio')) > 0]
            responses = {}
            for link in responses_link:
                text = prepare_response_from_text(link.get_text())

                responses[text] = [a.get('href') for a in link.find_all(
                    'a') if a.get('href') != None and '.mp3' in a.get('href')]
...
def return_response_media_url(hero, response, filename='dota.dat'):
    if os.path.exists(filename) and os.path.isfile(filename):
        response_data = load_data_file(filename)
        hero_ = beautify_hero_name(encode_hero_name(hero))
        for hero in response_data:

            if hero['name'] == hero_:

                hero_response_data = hero['response_data']
                for k, v in hero_response_data.items():
                    if k == response:

                        return v[0]

    else:
        hero = encode_hero_name(hero)
